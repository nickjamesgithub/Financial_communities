...
ax1.plot(grid, ratio_xy, color=color)
ax1.tick_params(axis='y', labelcolor=color)
for j in range(len(exit_short_x_decision)):
    ax1.axvspan(int(grid[short_x_decision[j]]), int(grid[exit_short_x_decision[j]]), color="red", label="Short_X",alpha=0.1)
for k in range(len(exit_long_x_decision)):
    ax1.axvspan(int(grid[long_x_decision[k]]), int(grid[exit_long_x_decision[k]]), color="green", label="Long_X", alpha=0.1)
ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
color = 'tab:blue'
ax2.set_ylabel('strategy-profit', color=color)  # we already handled the x-label with ax1
ax2.plot(grid, money_list_cs_fix, color=color)
ax2.tick_params(axis='y', labelcolor=color)
fig.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig("Strategy_profit_vs_ratio")
plt.show()

plt.plot(grid, money_list_cs_fix, label="Strategy_profit")
